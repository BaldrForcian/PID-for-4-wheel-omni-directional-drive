...
if(digitalRead(47) == HIGH){                      //on switch    
  ...
  if (kotZ > 45){                                 //if the yaw angle is bigger than 45 degrees we turn it clockwise
    cw();
  }
  else if(kotZ < -45){                            //if the yaw angle is smaller than -45 degrees we turn it couterclockwise
    ccw();
  }
  else{                                           //for all angles between -45 and 45 we move in a direction with PID
    forward();                                    //movements in all directions
    //backwards();
    //rightp();
    //leftp();
    //forwardright();
